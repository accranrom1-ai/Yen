    ...
    if user["state"] == "NHAP_MA_XAC_NHAN":
        ...
        try:
          markup_admin = types.InlineKeyboardMarkup()
            markup_admin.row(
                types.InlineKeyboardButton(f"✅ Duyệt {ticket_id}", callback_data=f"apprut_{ticket_id}"),
                types.InlineKeyboardButton(f"❌ Từ Chối", callback_data=f"rejrut_{ticket_id}")
            )
            bot.send_message(
                ADMIN_ID if ADMIN_ID != 0 else message.chat.id, 
                f"🔔 <b>THÔNG BÁO RÚT TIỀN MỚI</b>\n🆔 Mã lệnh: {ticket_id}\n👤 User: @{user.get('username')} (ID: `{user_id}`)\n💵 Số tiền: <b>{so_tien_rut:,} VNĐ</b>\n🏦 Thông tin: `{text}`", 
...
# --- KHỞI CHẠY WEB SERVER HOÀN HẢO CHO RENDER ---
if __name__ == '__main__':
    # Xoá Webhook cũ và tạo lại để nhận tín hiệu Webhook từ Render
    bot.remove_webhook()
    time.sleep(0.5)
